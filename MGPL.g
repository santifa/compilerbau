...
attrAss	: LDF ASSIGN expr -> ^(ATTR ["ATTR"] LDF ASSIGN expr);
block       	: animBlock 
	| eventBlock
	;
...
expr 	:  OPARAN! expr CPARAN! |(op (OPARAN! expr CPARAN!)? )* ;
...
unary	: (NOT^)? atom ;//| MINUS atom;//(NUMBER | var  (TOUCHES^ var)?) ;
atom 	: NUMBER 
	| var  (TOUCHES^ var)?
	;
